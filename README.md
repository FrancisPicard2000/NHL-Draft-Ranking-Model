...
It took one hour and thirty minutes to run the script on my machine with those inputs. The dataset obtained from the data collection has 914 player records over 54 different statistics. To have the same train/test split for performance comparability, please use sklearn's train_test_split with stratification on the target and a random state of 22. For cross-validation, use sklearn's StratifiedKFold with 5 splits and no shuffle.
  

  
## Results
To this point, the best model is a decision tree using SMOTE, a gini split criterion, a max
depth of 9, 10 minimum samples to split a node, and a number of features to find the best split equal to log2 of the total number of features. It had a mean Weighted Cohen's Kappa score of 0.117 during cross-validation and a mean Weighted Cohen's Kappa score of 0.129 on the testing set.
